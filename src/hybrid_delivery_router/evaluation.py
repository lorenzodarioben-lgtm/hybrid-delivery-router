...
def create_system() -> tuple[
    BoxHillDeliveryMap, FuzzySpeedController, HybridPlanner, ReactiveAgent
]:
...
def sensitivity_analysis(
    levels=(0.0, 2.0, 5.0, 8.0, 10.0), start: str = DEFAULT_START, goal: str = DEFAULT_GOAL
) -> list[dict]:
    env, controller, planner, agent = create_system()
    rows = []
...
def phase_aware_breakdown(
    journey, env: BoxHillDeliveryMap, controller: FuzzySpeedController
) -> list[dict]:
    rows = []
